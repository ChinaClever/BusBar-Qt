#include "autosetaddress.h"
#include "ui_autosetaddress.h"

AutoSetAddress::AutoSetAddress(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::AutoSetAddress)
{
    ui->setupUi(this);
    initLanguage();

    ui->progressBar->setValue(0);
    ui->progressBar_2->setValue(0);
    ui->progressBar_3->setValue(0);
    ui->progressBar_4->setValue(0);
    setProcessBarColor(ui->progressBar,"green");
    setProcessBarColor(ui->progressBar_2,"green");
    setProcessBarColor(ui->progressBar_3,"green");
    setProcessBarColor(ui->progressBar_4,"green");
    for(int i = 0 ; i < BUS_NUM ; ++i){
        mCurBoxNum[i] = 0;
        sDataPacket *shm = get_share_mem();
        char *name = shm->data[i].busName;
        QString str = name;
        updateBusNameSlot(i , str);
    }
}

AutoSetAddress::~AutoSetAddress()
{
    delete ui;
}
void AutoSetAddress::initLanguage()
{
    if(gLanguage == 0){
        ui->autoSet1Btn->setText("自动分配1");
        ui->autoSet2Btn->setText("自动分配2");
        ui->autoSet3Btn->setText("自动分配3");
        ui->autoSet4Btn->setText("自动分配4");
    }else{
        ui->autoSet1Btn->setText("Automatic allocation 1");
        ui->autoSet2Btn->setText("Automatic allocation 2");
        ui->autoSet3Btn->setText("Automatic allocation 3");
        ui->autoSet4Btn->setText("Automatic allocation 4");
    }
}
void AutoSetAddress::on_autoSet1Btn_clicked()
{
    QString str;
    if(gLanguage == 0) str = tr("是否确定自动分配第1条母线串口地址?");
    else str = tr("Are you sure to automatically allocate the serial port address of the first bus?");
    QuMsgBox box(NULL, str);
    bool ret = box.Exec();
    if(ret){
        ui->autoSet1Btn->setEnabled(false);
        ui->progressBar->setValue(0);
        emit autoSetBus1Sig(1);
    }
}


void AutoSetAddress::on_autoSet2Btn_clicked()
{
    QString str;
    if(gLanguage == 0) str = tr("是否确定自动分配第2条母线串口地址?");
    else str = tr("Are you sure to automatically allocate the serial port address of the second bus?");
    QuMsgBox box(NULL, str);
    bool ret = box.Exec();
    if(ret){
        ui->autoSet2Btn->setEnabled(false);
        ui->progressBar_2->setValue(0);
        emit autoSetBus2Sig(2);
    }
}

void AutoSetAddress::on_autoSet3Btn_clicked()
{
    QString str;
    if(gLanguage == 0) str = tr("是否确定自动分配第3条母线串口地址?");
    else str = tr("Are you sure to automatically allocate the serial port address of the third bus?");
    QuMsgBox box(NULL, str);
    bool ret = box.Exec();
    if(ret){
        ui->autoSet3Btn->setEnabled(false);
        ui->progressBar_3->setValue(0);
        emit autoSetBus3Sig(3);
    }
}


void AutoSetAddress::on_autoSet4Btn_clicked()
{
    QString str;
    if(gLanguage == 0) str = tr("是否确定自动分配第4条母线串口地址?");
    else str = tr("Are you sure to automatically allocate the serial port address of the 4th bus?");
    QuMsgBox box(NULL, str);
    bool ret = box.Exec();
    if(ret){
        ui->autoSet4Btn->setEnabled(false);
        ui->progressBar_4->setValue(0);
        emit autoSetBus4Sig(4);
    }
}

void AutoSetAddress::setProgressbarPowValue(QProgressBar *bar,int value, int index)
{
    int min = 2;
    int max = getBoxNum(index)==-1?18:getBoxNum(index);
    max += 2;
    if(max - min > 0)
    {
        int ret = (value - min)*100.0/(max - min);
        bar->setValue(ret);
        if( max == value ) {
            QString type = QString(tr("自动分配地址"));
            if(gLanguage == 1) type = QString(tr("Automatic allocation address"));
            QString msg = QString(tr("第%1条母线自动分配串口地址成功 ！").arg(index+1));
            QString en = "first";
            switch(index+1)
            {
            case 1:{
                    ui->autoSet1Btn->setEnabled(true);
                    QPalette pa;
                    pa.setColor(QPalette::WindowText, Qt::green);
                    ui->statuslab1->setPalette(pa);
                    en = "first";
                }break;
            case 2:{
                    ui->autoSet2Btn->setEnabled(true);
                    QPalette pa;
                    pa.setColor(QPalette::WindowText, Qt::green);
                    ui->statuslab2->setPalette(pa);
                    en = "second";
            }break;
            case 3:{
                    ui->autoSet3Btn->setEnabled(true);
                    QPalette pa;
                    pa.setColor(QPalette::WindowText, Qt::green);
                    ui->statuslab3->setPalette(pa);
                    en = "third";
            }break;
            case 4:{
                    ui->autoSet4Btn->setEnabled(true);
                    QPalette pa;
                    pa.setColor(QPalette::WindowText, Qt::green);
                    ui->statuslab4->setPalette(pa);
                    en = "4th";
            }break;
            }
            if(gLanguage == 1) msg = QString(tr("The %1 bus is automatically assigned the serial port address successfully !")).arg(en);
            db_operation_obj(index)->insertOperation(type , msg);
        }
        setProcessBarColor(bar , "green");
    }else
        bar->setValue(0);
}

void AutoSetAddress::setProcessBarColor(QProgressBar *bar, QString color)
{
    QString str = "QProgressBar{border:1px solid #000000;height:20;background:transparent;text-align:center;color:rgb(134,68,54);border-radius:10px;}";
    str += QString(" QProgressBar::chunk{border-radius:10px;border:1px solid black;background-color:%1;margin:0.5px;}").arg(color);
    bar->setStyleSheet(str);
}

void AutoSetAddress::getNumAndIndexSlot(int index , int curnum)
{
    int num = curnum >= 2 ? curnum : 2;
    int in = index + 1;
    switch(in)
    {
    case 1:
    {
        setProgressbarPowValue(ui->progressBar , num , index);
        mCurBoxNum[0] = num;
        if(num > 2){
            if(gLanguage == 0) ui->statuslab1->setText(QString(tr("设置地址%1成功")).arg(num-1));
            else ui->statuslab1->setText(QString(tr("Set address %1 successfully")).arg(num-1));
            QPalette pa;
            pa.setColor(QPalette::WindowText, Qt::black);
            ui->statuslab1->setPalette(pa);
        }
    }break;
    case 2:
    {
        setProgressbarPowValue(ui->progressBar_2 , num , index);
        mCurBoxNum[1] = num;
        if(num > 2){
            if(gLanguage == 0) ui->statuslab2->setText(QString(tr("设置地址%1成功")).arg(num-1));
            else ui->statuslab2->setText(QString(tr("Set address %1 successfully")).arg(num-1));
            QPalette pa;
            pa.setColor(QPalette::WindowText, Qt::black);
            ui->statuslab2->setPalette(pa);
        }
    }break;
    case 3:
    {
        setProgressbarPowValue(ui->progressBar_3 , num , index);
        mCurBoxNum[2] = num;
        if(num > 2){
            if(gLanguage == 0) ui->statuslab3->setText(QString(tr("设置地址%1成功")).arg(num-1));
            else ui->statuslab3->setText(QString(tr("Set address %1 successfully")).arg(num-1));
            QPalette pa;
            pa.setColor(QPalette::WindowText, Qt::black);
            ui->statuslab3->setPalette(pa);
        }
    }break;
    case 4:
    {
        setProgressbarPowValue(ui->progressBar_4 , num , index);
        mCurBoxNum[3] = num;
        if(num > 2){
            if(gLanguage == 0) ui->statuslab4->setText(QString(tr("设置地址%1成功")).arg(num-1));
            else ui->statuslab4->setText(QString(tr("Set address %1 successfully")).arg(num-1));
            QPalette pa;
            pa.setColor(QPalette::WindowText, Qt::black);
            ui->statuslab4->setPalette(pa);
        }
    }break;
    }
}

void AutoSetAddress::getDelaySlot(int index)
{
    int in = index + 1;
    QString type = QString(tr("自动分配地址"));
    if(gLanguage == 1) type = QString(tr("Automatic allocation address"));
    QString msg = QString(tr("第%1条母线始端箱未发出命令 !").arg(in));
    QString en = "first";
    switch(in)
    {
    case 1:
    {
        en = "first";
        setProcessBarColor(ui->progressBar , "red");
        if(mCurBoxNum[0] == 0){
            if(gLanguage == 1) msg = QString(tr("The feeder box of %1 bus did not issue a command !").arg(en));
        }else if(mCurBoxNum[0] >= 2 && mCurBoxNum[0] <= 20 ){
            if(gLanguage == 0) msg = QString(tr("第%2条母线设置地址%1失败 !")).arg(mCurBoxNum[0]).arg(in);
            else msg = QString(tr("The %2 bus set address %1 failed !")).arg(mCurBoxNum[0]).arg(en);
        }
        ui->statuslab1->setText(msg);
        ui->autoSet1Btn->setEnabled(true);
        QPalette pa;
        pa.setColor(QPalette::WindowText, Qt::red);
        ui->statuslab1->setPalette(pa);
        mCurBoxNum[0] = 0;
    }break;
    case 2:
    {
        en = "second";
        setProcessBarColor(ui->progressBar_2 , "red");
        if(mCurBoxNum[1] == 0){
            if(gLanguage == 1) msg = QString(tr("The feeder box of %1 bus did not issue a command !").arg(en));
        }else if(mCurBoxNum[1] >= 2 && mCurBoxNum[1] <= 20 ){
            if(gLanguage == 0) msg = QString(tr("第%2条母线设置地址%1失败 !")).arg(mCurBoxNum[1]).arg(in);
            else msg = QString(tr("The %2 bus set address %1 failed !")).arg(mCurBoxNum[1]).arg(en);
        }
        ui->statuslab2->setText(msg);
        ui->autoSet2Btn->setEnabled(true);
        QPalette pa;
        pa.setColor(QPalette::WindowText, Qt::red);
        ui->statuslab2->setPalette(pa);
        mCurBoxNum[1] = 0;
    }break;
    case 3:
    {
        en = "third";
        setProcessBarColor(ui->progressBar_3 , "red");
        if(mCurBoxNum[2] == 0){
            if(gLanguage == 1) msg = QString(tr("The feeder box of %1 bus did not issue a command !").arg(en));
        }else if(mCurBoxNum[2] >= 2 && mCurBoxNum[2] <= 20 ){
            if(gLanguage == 0) msg = QString(tr("第%2条母线设置地址%1失败 !")).arg(mCurBoxNum[2]).arg(in);
            else msg = QString(tr("The %2 bus set address %1 failed !")).arg(mCurBoxNum[2]).arg(en);
        }
        ui->statuslab3->setText(msg);
        ui->autoSet3Btn->setEnabled(true);
        QPalette pa;
        pa.setColor(QPalette::WindowText, Qt::red);
        ui->statuslab3->setPalette(pa);
        mCurBoxNum[2] = 0;
    }break;
    case 4:
    {
        en = "4th";
        setProcessBarColor(ui->progressBar_4 , "red");
        if(mCurBoxNum[3] == 0){
            if(gLanguage == 1) msg = QString(tr("The feeder box of %1 bus did not issue a command !")).arg(en);
        }else if(mCurBoxNum[3] >= 2 && mCurBoxNum[3] <= 20 ){
            if(gLanguage == 0) msg = QString(tr("第%2条母线设置地址%1失败 !")).arg(mCurBoxNum[3]).arg(in);
            else msg = QString(tr("The %2 bus set address %1 failed !")).arg(mCurBoxNum[3]).arg(en);
        }
        ui->statuslab4->setText(msg);
        ui->autoSet4Btn->setEnabled(true);
        QPalette pa;
        pa.setColor(QPalette::WindowText, Qt::red);
        ui->statuslab4->setPalette(pa);
        mCurBoxNum[3] = 0;
    }break;
    }
    db_operation_obj(index)->insertOperation(type , msg);
}

void AutoSetAddress::updateBusNameSlot(int index,QString &name)
{
    switch(index)
    {
        case 0: ui->buslab1->setText(name);break;
        case 1: ui->buslab2->setText(name);break;
        case 2: ui->buslab3->setText(name);break;
        case 3: ui->buslab4->setText(name);break;
    }
}
