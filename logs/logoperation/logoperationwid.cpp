 ...
 *      Author: Lzy
 */
#include "logoperationwid.h"
#include "ui_logoperationwid.h"
#include "dboperation.h"
#include "logoperationexportdlg.h"

LogOperationWid::LogOperationWid(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::LogOperationWid)
{
    ui->setupUi(this);
...
LogOperationWid::~LogOperationWid()
{
    delete ui;
}


//void LogOperationWid::initScrollArea()
//{
...
//bool LogOperationWid::eventFilter(QObject *obj, QEvent *event)
//{
...
void LogOperationWid::initFunSLot()
{
...
void LogOperationWid::initBtnBar()
{
...
QString LogOperationWid::getTableName(int id)
{
    QString str = db_operation_obj(id)->tableName();
    if(gLanguage == 0) str = db_operation_obj(id)->tableName();
    else str = db_operation_obj_en(id)->tableName();
    return str;
}

void LogOperationWid::initTableSlot(int id)
{
    mid = id;
    m_table = getTableName(id);
    this->refreshTable(m_table);

    if(gLanguage == 0) mHeadList << tr("编号") << tr("日期") << tr("时间") << tr("操作类型")<< tr("详细描述") ;
    else mHeadList << tr("NO.") << tr("Date") << tr("Time") << tr("Operation type")<< tr("Detailed description") ;
    model->setHeaders(mHeadList);
}


bool LogOperationWid::refreshTable(const QString &table)
{
    ...
    if(ret) {
        ...
        ui->tableView->setColumnWidth(4,472); //设置宽度
    }
    return  ret;
}

void LogOperationWid::clearTableSlot()
{
#if SQL_DEL_MODE
    int row = model->model->rowCount();
    DbOperation* db = db_operation_obj(mid);
    DbOperation* dben = db_operation_obj_en(mid);
    if(mCount++ % 2 ==0)
    {
...
void LogOperationWid::refreshSlot()
{
    initTableSlot(mid);
}

void LogOperationWid::doubleSlot(QModelIndex)
{
    QString str;
 ...
 */
void LogOperationWid::exportSlot(int mode)
{
    if(mode == 0) {
        LogOperationExportDlg dlg(this);
        dlg.set(mid);
        dlg.exec();
    }
}
