...
LogSystemExportThread::LogSystemExportThread(QObject *parent) : QThread(parent)
{
    mExcelStr = gExcelExportStr;
}

LogSystemExportThread::~LogSystemExportThread()
{
    wait();
}
...
bool LogSystemExportThread::readDb()
{
    ...
    QStringList heads;
    if(gLanguage == 0) heads << "No." << "Date" << "Time" << tr("详细描述");
    else heads << "No." << "Date" << "Time"  << tr("Detailed description");
    mList << heads;

    QVector<DbSystemItem> items = mSystem->selectByDate(mExcelStr->start, mExcelStr->end);
    if(items.size() > 0)
    {
...
void LogSystemExportThread::run()
{
    bool ret = readDb();
    emit exportSig(ret);
}
