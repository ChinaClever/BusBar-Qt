 ...
 *      Author: Lzy
 */
#include "logsystemwid.h"
#include "ui_logsystemwid.h"
#include "dbsystem.h"
#include "logsystemexportdlg.h"

LogSystemWid::LogSystemWid(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::LogSystemWid)
{
    ui->setupUi(this);
...
LogSystemWid::~LogSystemWid()
{
    delete ui;
}


//void LogSystemWid::initScrollArea()
//{
...
//bool LogSystemWid::eventFilter(QObject *obj, QEvent *event)
//{
...
QString LogSystemWid::getTableName()
{
    QString str = db_system_obj()->tableName();
    if(gLanguage == 0) str = db_system_obj()->tableName();
    else str = db_system_obj_en()->tableName();
    return str;
}

void LogSystemWid::initTableSlot()
{
    m_table = getTableName();
    this->refreshTable(m_table);

    if(gLanguage == 0) mHeadList << tr("编号") << tr("日期") << tr("时间") << tr("详细描述") ;
    else mHeadList << tr("NO.") << tr("Date") << tr("Time") << tr("Detailed description") ;
    model->setHeaders(mHeadList);
}


bool LogSystemWid::refreshTable(const QString &table)
{
...
//        ui->tableView->setColumnWidth(3,120); //设置宽度
        ui->tableView->setColumnWidth(3,592); //设置宽度//90+85+600=775
    }
    return  ret;
}

void LogSystemWid::clearTableSlot()
{
#if SQL_DEL_MODE
    int row = model->model->rowCount();
    DbSystem* db = db_system_obj();
    DbSystem* dben = db_system_obj_en();
    if(mCount++ % 2 ==0)
    {
...
void LogSystemWid::refreshSlot()
{
    initTableSlot();
}

void LogSystemWid::doubleSlot(QModelIndex)
{
    QString str;
 ...
 */
void LogSystemWid::exportSlot(int mode)
{
    if(mode == 0) {
        LogSystemExportDlg dlg(this);
        dlg.set();
        dlg.exec();
    }
}
